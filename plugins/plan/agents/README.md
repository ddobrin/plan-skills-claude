...
The `initialPrompt` is the field with no equivalent in the skills form. Claude Code auto-submits it as the first user turn **only when the agent runs as the main session agent** (`claude --agent <name>`). When an agent is dispatched as a subagent with the `Agent` tool it does not run, and the agent cannot ask the dispatching user anything; the body below the frontmatter is the whole contract in that mode. Its job in main-session mode is to *establish state and confirm the target* before acting.
...
The natural entry point for an end-to-end run is **`supervisor`** ("be the supervisor", "run the swarm"); the role and validator agents can also be dispatched standalone for a single phase (e.g. "validate this spec" → `spec-validator`, "plan this milestone" → `architect`). When an agent is launched with `claude --agent`, its `initialPrompt` runs first — expect it to orient and confirm the target before doing any work. Under `Agent` tool dispatch the body alone governs.
