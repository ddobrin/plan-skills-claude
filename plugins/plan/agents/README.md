...
> Both families use the same role names, including the orchestrator, **`supervisor`**.
...
| `model` | The model the subagent runs on. Every agent uses `inherit` (run on the caller's model). No agent sets `effort`, so each runs at the session's effort level. |
| `color` | The agent's color in the subagent UI (blue architects, red validators, green engineer, purple product/deliberators, cyan orchestrator/recap, yellow auditor). |
...
### Orientation — orient before acting

Each role's body carries its orientation step: *establish state and confirm the target* before acting. The supervisor reads the roadmap and every milestone and reports the next action before dispatching anything; the architect finds specs with no `plan.md`; the validators and deliberators pin down the artifact (or the `BASE..HEAD` diff range) in the first step of their process.

A dispatched subagent cannot ask the user anything. When the target is ambiguous, an agent stops and puts the question in its final report, and the supervisor relays it; the engineer returns a `blocked` status with a proposed plan change. Launched as the main session (`claude --agent <name>`), the same agent asks the user directly.

> Plugin agents do not support the `initialPrompt` frontmatter field, so orientation lives in the body, which loads in every mode.
...
`model: inherit` · `color: cyan` · `tools: all` — The Project Manager and Guardian of the Protocol. **Does no work itself**; it runs the state machine, dispatching the other agents in the correct order and enforcing the lifecycle above. 

- **Owns:** protocol enforcement, artifact management, human gating, the git protocol.
- **Orientation:** read the roadmap, inventory every active milestone, determine the current phase, report *(milestone, next action, target agent)* — then **stop and wait** for the user before dispatching anything.
...
`model: inherit` · `color: purple` · `tools: Read, Write, Edit, Glob, Grep, AskUserQuestion` (no `Bash` — read-only on code) — Translates raw, ambiguous human ideas into rigorous, testable specifications, and owns the master roadmap.
...
- **Orientation:** read `plans/research/*.md` and the roadmap first; if a feature was named, start grilling; otherwise ask what to specify. Never write `spec.md` until the critical ambiguities are resolved.

#### 2·alt. `visual-product-owner` — The Visual Product Owner (Spec author + Renderer)
`model: inherit` · `color: purple` · `tools: … + Bash` — A **drop-in alternative to `product-owner`**. Runs the identical Grill Loop and writes the same `spec.md`, then renders that spec as a self-contained, browsable HTML document (`visual-spec.html`).
...
- **Orientation:** same orientation as `product-owner`, plus: render `visual-spec.html` **only after `spec.md` is complete** — the HTML is a derived view, so no requirement may live only in it.
...
- **Orientation:** find specs with no `plan.md`, confirm the target, and **investigate the code before writing** — blind planning is forbidden.
...
- **Orientation:** same orientation as `architect`, plus: produce `plan.md` **first**, then render the HTML from it; no decision may live only in the HTML.
...
- **Doctrine:** no untested changes; Red → Green → Refactor; characterization tests + seams for legacy code (Feathers); small increments that keep the build green; the simplest code that fits existing patterns; fail fast; no tidying outside the task.
- **Tracks progress** by checking off todos directly in `plan.md`; uses `git mv` to preserve history.
- **Orientation:** *do not write code until you have a plan and a task* — confirm the `plan.md` and `Task [X.Y]`, recite the step to confirm scope, then proceed under TDD. Never expand scope; never commit.
...
- **Orientation:** identify the plan and the just-completed tasks, verify statically then dynamically (build + tests), write the PASS/FAIL report — never fix code, commit only on green + explicit approval.
...
- **Orientation:** identify the `spec.md` and milestone moniker, note context the spec depends on but doesn't restate, then dispatch the 3 skeptics and write `adversarial-reviews/spec-validation.md`.
...
- **Orientation:** identify the `plan.md` and the repository root the skeptics must read, then dispatch the 3 codebase-reading skeptics and write `adversarial-reviews/plan-validation.md`.
...
- **Orientation:** establish the `BASE..HEAD` diff range (`git rev-parse origin/main` / `HEAD`) and a one-line statement of what the change claims, confirm the mode, then dispatch the 3 skeptics and write `adversarial-reviews/implementation-validation.md`.
...
| **Per-role runtime config** | Implicit | Explicit frontmatter: `model`, `color`, `tools`. |
| **Invocation** | `Skill` tool | `Agent` tool (`subagent_type`), auto-delegation from `description`, or `claude --agent <name>`. |
...
A typical end-to-end run. The supervisor offers each gate marked *(offered)* or *(optional)* at its phase boundary and runs it only if the user accepts:
...
5. *(offered)* **`plan-validator`** attacks the plan against the real codebase; the `first_domino` and confirmed fixes are applied.
6. **🛑 Human review gate** — the user reviews `spec.md` + `plan.md` and types "approve".
7. **`engineer`** (one per independent task, in parallel) implements each group under TDD; **`auditor`** verifies each group and writes an audit report.
...
1. **Explicitly, with the `Agent` tool** — set `subagent_type` to `plan:{name}` (e.g. `plan:supervisor`, `plan:architect`, `plan:spec-validator`).
